...
Limits bound recursion, input, line count, and edit count. The core does no IO;
the JS host only reads bounded UTF-8 data and writes output/exit status. It never
...
4. CI check/build/test across four MoonBit backends; CLI uses Node 24.
